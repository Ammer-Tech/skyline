...
func NewServer(cfg *config.SkylineConfig) *SkylineServer {
	...
	srv := smtpd.Server{Addr: addr, Handler: mailHandler, Appname: appname, Hostname: hostname,
		AuthHandler: authHandler, AuthRequired: true, AuthMechs: map[string]bool{
			"CRAM-MD5": false,
			"PLAIN":    true,
		},
	}
	...
	globalProxyBackend := skybackend.NewBackend(cfg)
	sndr = globalProxyBackend.Sender

	return &SkylineServer{
		ctx:          ctx,
		metrics:      metricsServer(cfg.MetricsPort),
		mailServer:   &srv,
		proxyBackend: globalProxyBackend,
	}
}
